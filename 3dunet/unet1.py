...
class encoder(nn.Module):
    def __init__(self,in_planes):
        ...
        self.layer1=nn.Sequential(ConvBlock([in_planes, 16, 32]))
        self.p1=nn.MaxPool3d(kernel_size=2, stride=2)
        self.layer2=nn.Sequential(ConvBlock([32, 32, 64]))
        self.p2=nn.MaxPool3d(kernel_size=2, stride=2)
        self.layer3=nn.Sequential(ConvBlock([64, 64, 128]))
        self.p3=nn.MaxPool3d(kernel_size=2, stride=2)
        self.layer4=nn.Sequential(ConvBlock([128, 128, 256]))
...
class decoder(nn.Module):
    def __init__(self,models):
        ...
        self.layer2=nn.Sequential(ConvBlock([384, 128, 128]),
                                  nn.ConvTranspose3d(256, 256, kernel_size=2, stride=2,padding=1))
        self.layer3=nn.Sequential(ConvBlock([192, 64, 64]),
                                  nn.ConvTranspose3d(128, 128, kernel_size=2, stride=2,padding=1))
        self.layer4=nn.Sequential(ConvBlock([96, 32, 32]))
        self.conv1 = nn.Conv3d(32, 3, kernel_size=3, stride=1, bias=False)
        self.models = models
...
net=encoder(1).to("cuda:0")
inputs = torch.randn(1,1,333,512,512).to("cuda:0")
