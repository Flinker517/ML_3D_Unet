...
from data import FracNetInferenceDataset,Window,MinMaxNorm
from model.unet import UNet
from model.unet2 import my3dunet
...
def _predict_single_image(model, dataloader, prob_thresh,bone_thresh, size_thresh):
    ...
    with torch.no_grad():
        for _, sample in enumerate(dataloader):
            ...
            for i in range(len(centers)):
                ...
                cur_pred_patch = pred[center_x - crop_size // 2:center_x + crop_size // 2, center_y - crop_size // 2:center_y + crop_size // 2, center_z - crop_size // 2:center_z + crop_size // 2]
                pred[center_x - crop_size // 2:center_x + crop_size // 2, center_y - crop_size // 2:center_y + crop_size // 2, center_z - crop_size // 2:center_z + crop_size // 2] = np.where(cur_pred_patch > 0, np.mean((output[i], cur_pred_patch), axis=0), output[i])
    pred = _post_process(pred, dataloader.dataset.image, prob_thresh,bone_thresh, size_thresh)
    return pred
...
def predict(args):
    ...
    transforms = [Window(-200, 1000),MinMaxNorm(-200, 1000)]
    ...
    for image_id, image_path in zip(image_id_list, image_path_list):
        ...
        dataloader = FracNetInferenceDataset.get_dataloader(dataset, batch_size, num_workers)
        pred_arr = _predict_single_image(model, dataloader, prob_thresh, bone_thresh, size_thresh)
        pred_image, pred_info = _make_submission_files(pred_arr, image_id, dataset.image_affine)
